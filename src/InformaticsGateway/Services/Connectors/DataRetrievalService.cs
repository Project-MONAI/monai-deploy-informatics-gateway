...
namespace Monai.Deploy.InformaticsGateway.Services.Connectors
{
    internal class DataRetrievalService : IHostedService, IMonaiService, IDisposable
    {
            ...
            while (!cancellationToken.IsCancellationRequested)
            {
                ...
                try
                {
                    ...
                    using (_logger.BeginScope(new LoggingDataDictionary<string, object> { { "TransactionId", request.TransactionId } }))
                    {
                        ...
                        await repository.UpdateAsync(request, InferenceRequestStatus.Success, cancellationToken).ConfigureAwait(false);
        ...
        private async Task ProcessRequest(InferenceRequest inferenceRequest, CancellationToken cancellationToken)
        {
            ...
            await NotifyNewInstance(inferenceRequest, retrievedFiles).ConfigureAwait(false);
        }
        ...
        private async Task RestoreExistingInstances(InferenceRequest inferenceRequest, Dictionary<string, FileStorageMetadata> retrievedInstances, CancellationToken cancellationToken)
        {
            ...
            var repository = _rootScope.ServiceProvider.GetService<IStorageMetadataRepository>() ?? throw new ServiceNotFoundException(nameof(IStorageMetadataRepository));
            var files = await repository.GetFileStorageMetdadataAsync(inferenceRequest.TransactionId, cancellationToken).ConfigureAwait(false);
