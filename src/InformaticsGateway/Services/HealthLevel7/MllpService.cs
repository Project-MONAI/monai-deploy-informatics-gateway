        ...
        private async Task OnDisconnect(IMllpClient client, MllpClientResult result)
        {
            ...
            try
            {
                foreach (var message in result.Messages)
                {
                    ...
                    var payloadId = await _payloadAssembler.QueueAsync(client.ClientId.ToString(), hl7Fileetadata, new DataOrigin { DataService = DataService.HL7, Source = client.ClientIp, Destination = FileStorageMetadata.IpAddress() }).ConfigureAwait(false);
