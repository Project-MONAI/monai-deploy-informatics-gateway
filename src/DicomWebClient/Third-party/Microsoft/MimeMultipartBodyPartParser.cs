    ...
    internal class MimeMultipartBodyPartParser : IDisposable
    {
        ...
        public IEnumerable<MimeBodyPart> ParseBuffer(byte[] data, int bytesRead)
        {
            ...
            while (_mimeParser.CanParseMore(bytesRead, bytesConsumed))
            {
                ...
                {
                    CleanupCurrentBodyPart();
                    throw new InvalidOperationException($"Error parsing MIME multipart message byte {bytesConsumed} of data segment {data}.");
                }
                ...
                foreach (ArraySegment<byte> part in _parsedBodyPart)
                {
                    ...
                    if (_bodyPartHeaderStatus != ParserState.Done)
                    {
                        ...
                        {
                            CleanupCurrentBodyPart();
                            throw new InvalidOperationException($"Error parsing MIME multipart body part header byte {headerConsumed} of data segment {part.Array}.");
                        }
                    }
                    else
                    {
        ...
        private static string ValidateArguments(HttpContent content, long maxMessageSize, bool throwOnError)
        {
            ...
            if (maxMessageSize < MimeMultipartParser.MinMessageSize)
            {
                if (throwOnError)
                {
                    throw new ArgumentOutOfRangeException("maxMessageSize", $"Value must be greater than or equal to {MimeMultipartParser.MinMessageSize}.");
                }
                else
                {
                    return null;
            ...
            if (contentType == null)
            {
                if (throwOnError)
                {
                    throw new ArgumentException($"Invalid '{typeof(HttpContent).Name}' instance provided. It does not have a content-type header value. '{typeof(HttpContent).Name}' instances must have a content-type header starting with 'multipart/'.", "content");
                }
                else
                {
                    return null;
            ...
            if (!contentType.MediaType.StartsWith("multipart", StringComparison.OrdinalIgnoreCase))
            {
                if (throwOnError)
                {
                    throw new ArgumentException($"Invalid '{typeof(HttpContent).Name}' instance provided. It does not have a content type header starting with 'multipart/'.", "content");
                }
                else
                {
                    return null;
            ...
            if (boundary == null)
            {
                if (throwOnError)
                {
                    throw new ArgumentException($"Invalid '{typeof(HttpContent).Name}' instance provided. It does not have a 'multipart' content-type header with a 'boundary' parameter.", "content");
                }
                else
                {
                    return null;
