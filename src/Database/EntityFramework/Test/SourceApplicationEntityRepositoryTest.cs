...
namespace Monai.Deploy.InformaticsGateway.Database.EntityFramework.Test
{
    ...
    public class SourceApplicationEntityRepositoryTest
    {
        ...
        public async Task GivenASourceApplicationEntity_WhenAddingToDatabase_ExpectItToBeSaved()
        {
            ...
            var actual = await _databaseFixture.DatabaseContext.Set<SourceApplicationEntity>().FirstOrDefaultAsync(p => p.Name.Equals(aet.Name)).ConfigureAwait(ConfigureAwaitOptions.ContinueOnCapturedContext);
        ...
        public async Task GivenASourceApplicationEntity_WhenRemoveIsCalled_ExpectItToDeleted()
        {
            ...
            var dbResult = await _databaseFixture.DatabaseContext.Set<SourceApplicationEntity>().FirstOrDefaultAsync(p => p.Name == "AET5").ConfigureAwait(ConfigureAwaitOptions.ContinueOnCapturedContext);
        ...
        public async Task GivenDestinationApplicationEntitiesInTheDatabase_WhenToListIsCalled_ExpectAllEntitiesToBeReturned()
        {
            ...
            var expected = await _databaseFixture.DatabaseContext.Set<SourceApplicationEntity>().ToListAsync().ConfigureAwait(ConfigureAwaitOptions.ContinueOnCapturedContext);
